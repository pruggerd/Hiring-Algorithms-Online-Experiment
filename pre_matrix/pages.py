...
class training(Page):
    ...
    def before_next_page(self):
        ...
        self.player.elapsed_time0 = round(time.time() - self.player.start_time0, 3)
        self.player.start_time1 = time.time()
        self.player.count1 = 1
...
class training2(Page):
    def vars_for_template(self):
        self.participant.vars['score0'] = max(round(90 - self.player.elapsed_time0, 3),0)
        return {
            'score0': self.participant.vars['score0'],
            'time0': round(self.player.elapsed_time0, 2),
            'count0': self.player.count0,
            'matrix_payoff0': self.player.matrix_payoff0,
            'time1': round(self.player.elapsed_time1, 2),
            'count1': self.player.count1,
            'count2': self.player.count2,
            'time2': round(self.player.elapsed_time2, 2),
        }

class training3(Page):
    pass

class training4(Page):
    pass
